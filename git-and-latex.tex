...
\maketitle % This causes the title/author information above to be typset neatly.

\begin{lem}[Geometric series]

	If z is a real number and \abs{z} < 1, then
...
\begin{proof}
	...
	For any positive integer $n$, let $S_n$ denote the partial sum,
	
	$$S_n = 1 + z + z^2 + \dots + z^n.$$
	
	\noindent Multiplying $S_n$ by $1 - z$ yields
	
	$$(1-z)S_n = (1+z+\dots+z^n)-(z+z^2+\dots+z^{n+1})=1-z^{n+1}.$$
	
	\noindent Therefore,
	
	$$S_n = \frac{1-z^{n+1}}{1-z}.$$
	
	\noindent Since $\abs{z} < 1$, we have $lim_{n \to \infty}z^{n+1} = 0$. It follows that $$\lim_{n\to\infty} S_n = \frac{1}{1-z}.$$
