...
The theorem is true. However, the proof provided for the theorem is invalid. To correct the invalid proof, we note that the contrary statement is written incorrectly since it did not negate the universal quantifier. 
Consequently, the incorrect proof only provided a particular counter-example to form a contradiction, i.e. that there \emph{exists} $a, b, c > 0$ such that 
...
The proof is incorrect. The logical flow of the incorrect proof is as follows. Firstly, it does induction on $m$ while holding $n=1$. Next, it does induction on $n$ while holding $m=1$. These
two inductions, considered separately, are valid. However, the final step of the proof claims that for \emph{every} $m,n \in \mathbb{N}$, and for every positive $x \in \mathbb{Q}$, there exist $a, b \in \mathbb{N}$ 
...
A way to make this proof more efficient is to note that proving $P(1,n)$ is unnecessary once $P(1,1)$ and $P(m,1)$ are proven. Thus, the proof for the base case for the second induction is unnecessary.
...
\begin{proof}
	...
	For the base case, when $n=1$, $a_1 = 1$ and $b_1 = 1$. The LHS of $L(1)$ is $(1+\sqrt{2})^1 &= 1 + 1\cdot\sqrt{2} &= a_1 + b_1\sqrt{2} =$ RHS. Similarly, for $R(1)$, LHS = $a_1^2 - 2b_1^2 = 1^2 - 2\cdot 1^2 = -1
	= (-1)^1 =$ RHS. Hence, since both $L(1)$ and $R(1)$ are true, the base case $C(1)$ is true.\\
	
	For the inductive step, assume $C(n), L(n), R(n)$ are true for some $n\in\mathbb{N}$. To show $C(n) \implies C(n+1)$, we first show $L(n) \implies L(n+1)$ and $R(n) \implies R(n+1)$:
	...
	Therefore, $L(n) \implies L(n+1)$ and $R(n) \implies R(n+1)$, and thus $C(n) \implies C(n+1)$. Because we know $C(1)$ is true, and $C(n+1)$ is true if $C(n)$ is true, by induction 
...
\begin{proof}
	...
	We observe from Lemma \ref{lem:powers-of-1+sqrt2} that the functions $a_{n+1} = a_n + 2b_n$ and $b_{n+1} = a_n + b_n$ are monotone increasing. Since $a_1 = b_1 = 1$, $a_{k+1} > a_k$ and $b_{k+1} > b_k$, we 
	...
	Since all the terms in $\frac{q - p(a_n)}{p(b_n)}$ are integers, the numerator and denominator are integers and therefore by definition we know that $\frac{x-a_n}{b_n} \in\mathbb{Q}$. However, from a previous proof, we know that $\sqrt2$ is 
...
\begin{proof}
	Let $P(x)$ be the statement: ``For each $x\in\mathbb{N}^0$, and for each integer $n\geq2$, there exists an eventually zero n-nary sequence $(a_j)_{j\in\mathbb{N}}$ such that 
	$$x = \sum_{j=0}^{\infty}{a_{j}n^j}."$$
	Note that, because the sequence is eventually zero, the series is a finite sum and converges.
	Base case: the case $P(0)$ when $x=0$ is trivially true for all $n>2$, with the empty sum given by the sequence in which every term is zero.
	
	Suppose that, for some integer $x \geq 0$, for every integer $k$ such that $0\leq k \leq x$, there is an eventually zero n-nary sequence $(a_{k,j})_{j\in\mathbb{N}^0}$, such that
	$$k = \sum_{j=0}^{\infty}{a_{k,j}n^j}$$
	is true for all $n\geq2$.\\
	
	If $x+1$ is divisible by $n$, then $0\leq \frac{x+1}{n}\leq x, \frac{x+1}{n}\in\mathbb{N}^0$, for all integer $n \geq 2$. By the inductive hypothesis, there exists a sequence $(a_{\frac{x+1}{n}},j)_{j\in\mathbb{N}^0}$,
	...
	If $x+1$ is not divisible by $n$, then let $R$ be the remainder of $\frac{x+1}{n}$. Since $R$ is the remainder, we know $1\leq R\leq n-1$. By the Quotient-Remainder theorem,
	we also know that $0\leq\frac{x+1-R}{n}\leq x$ and $n \mid (x+1-R)$ for all integer $n \geq 2$.
	By the inductive hypothesis, there exists a sequence $(a_{\frac{x+1-R}{n},j\right{)}_{n\in\mathbb{N}^0}$ which is eventually zero such that 
	...
	\noindent This new sequence is eventually zero because $(a_{\frac{x+1-R}{n},j\right{)}_{n\in\mathbb{N}^0}$ was eventually zero.
	...
	Because $P(0)$ is true for \emph{all} integer $n\geq2$, and if $P(x)$ is true for \emph{all} integer $n\geq2$ then $P(x+1)$ is true for \emph{all} integer $n\geq2$, by complete induction we know that $P(x)$ is true for all $x\in\mathbb{N}^0$ and for 
	all integer $n\geq2$.
